...
#ifndef KW_BLOCKSSTABLE_KW_IMICRO_BLOCK_WRITER_H_
#define KW_BLOCKSSTABLE_KW_IMICRO_BLOCK_WRITER_H_
...
enum KW_MICRO_BLOCK_MERGE_VERIFY_LEVEL
{
  KW_NONE = 0,
  KW_ENCODING = 1,
  KW_ENCODING_AND_COMPRESSION = 2,
  KW_ENCODING_AND_COMPRESSION_AND_WRITE_COMPLETE = 3,
};

// Some common interface of ObMicroBlockWriter and ObMicroBlockEncoder, not all features.
class KwIMicroBlockWriter
{
...
public:
  KwIMicroBlockWriter() :
    ...
    micro_block_merge_verify_level_(KW_MICRO_BLOCK_MERGE_VERIFY_LEVEL::KW_ENCODING_AND_COMPRESSION),
  ...
  virtual ~KwIMicroBlockWriter() {}
  virtual int append_row(const KwDatumRow &row) = 0;
  ...
  int build_micro_block_desc(KwMicroBlockDesc &micro_block_desc);
  ...
  static int64_t cal_row_checksum(const KwDatumRow &row, int64_t checksum)
  {
  ...
  static int cal_column_checksum(const KwDatumRow &row, int64_t *curr_micro_column_checksum)
  {
  ...
  void set_micro_block_merge_verify_level(const int64_t verify_level) { micro_block_merge_verify_level_ = verify_level; }
  // VIRTUAL_TO_STRING_KV(K_(row_count_delta), K_(micro_block_checksum), K_(contain_uncommitted_row),
  //     K_(max_merged_trans_version));

protected:
  inline void cal_row_stat(const KwDatumRow &row)
  {
  ...
  inline bool need_cal_row_checksum() const
  {
    return KW_MICRO_BLOCK_MERGE_VERIFY_LEVEL::KW_NONE < micro_block_merge_verify_level_;
  }
