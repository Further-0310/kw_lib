#ifndef KW_STORAGE_BLOCKSSTABLE_OB_ROW_READER_H_
#define KW_STORAGE_BLOCKSSTABLE_OB_ROW_READER_H_
// #include <stdint.h>
...
public:
  ...
  int init(
    ...
    //   const KwColClusterInfoMask &info_mask);
      const KwColClusterInfoMask &info_mask);
  void reset();
  int read_storage_datum(const int64_t column_idx, KwStorageDatum &datum);
  int sequence_read_datum(const int64_t column_idx, KwStorageDatum &datum);
//   int sequence_deep_copy_datums(const int64_t start_idx, KwStorageDatum *datum);
//   int read_cell_with_bitmap(
//       const int64_t start_idx,
//       const storage::ObTableReadInfo &read_info,
//       KwDatumRow &datum_row,
//       memtable::ObNopBitMap &nop_bitmap);
  inline int64_t get_sparse_col_idx(const int64_t column_idx);
...
private:
//   int sequence_deep_copy_datums_of_sparse(const int64_t start_idx, KwStorageDatum *datums);
//   int sequence_deep_copy_datums_of_dense(const int64_t start_idx, KwStorageDatum *datums);
  ...
  int read_column_from_buf(
    ...
    //   const KwRowHeader::SPECIAL_VAL special_val,
      const KwRowHeader::SPECIAL_VAL special_val,
      KwStorageDatum &datum);    
  int read_datum(const int64_t column_idx, KwStorageDatum &datum);
  inline uint8_t read_special_value(const int64_t column_idx) 
  {
    const int64_t index = column_idx >> 1; 
    const int64_t shift = (column_idx % 2) << 2;
    // const uint8_t sv = special_vals_[index];
    // const uint8_t ss = sv >> shift;
    // const uint8_t ret = ss & 0x0F;
    // return ret;
...
private:
  ...
  KwColClusterInfoMask::BYTES_LEN offset_bytes_;
  KwColClusterInfoMask::BYTES_LEN col_idx_bytes_;
  bool is_sparse_row_;
  bool is_inited_;
};
...
public:
    ...
    //   memtable::ObNopBitMap &nop_bitmap,
    //   bool &read_finished);
    int read_row_header(const char *row_buf, const int64_t row_len, const KwRowHeader *&row_header);
...
private:
    ...
    inline int analyze_row_header();
    inline int analyze_cluster_info();
    inline int read_specific_column_in_cluster(const int64_t store_idx, KwStorageDatum &datum);
    ...
    inline uint64_t get_cluster_offset(const int64_t cluster_idx) const;
    inline uint64_t get_cluster_end_pos(const int64_t cluster_idx) const;
    inline int analyze_info_and_init_reader(const int64_t cluster_idx);
protected:
    const char *buf_;
    int64_t row_len_;
    const KwRowHeader *row_header_;
    const void *cluster_offset_;
    const void *column_offset_;
    const void *column_idx_array_;
    KwClusterColumnReader cluster_reader_;
...
template<class T>
inline const T *KwRowReader::read(const char *row_buf, int64_t &pos)
{
