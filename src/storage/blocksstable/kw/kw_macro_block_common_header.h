...
#ifndef KW_MACRO_BLOCK_COMMON_HEADER_H_
#define KW_MACRO_BLOCK_COMMON_HEADER_H_
...
public:
  ...
  static_assert(
    static_cast<int64_t>(KwMacroBlockType::MaxMacroType) < 16,//common::OB_MAX_MACRO_BLOCK_TYPE,
    "MacroBlockType max value should less than 4 bits");

  KwMacroBlockCommonHeader();
  // DO NOT use virtual function. It is by design that sizeof(KwMacroBlockCommonHeader) equals to
  ...
  bool is_sstable_data_block() const { return KwMacroBlockType::SSTableData == attr_; }
  bool is_linked_macro_block() const { return KwMacroBlockType::LinkedBlock == attr_; }
  bool is_bloom_filter_data_block() const { return KwMacroBlockType::BloomFilterData == attr_; }
  bool is_sstable_index_block() const { return KwMacroBlockType::SSTableIndex ==  attr_; }
  bool is_sstable_macro_meta_block() const { return KwMacroBlockType::SSTableMacroMeta == attr_; }
  bool is_shared_macro_block() const { return KwMacroBlockType::SharedSSTableData == attr_; }
  ...
  KwMacroBlockType get_type() const { return static_cast<KwMacroBlockType>(attr_); }
  int32_t get_payload_size() const { return payload_size_; }
  int32_t get_payload_checksum() const { return payload_checksum_; }
  void set_attr(const KwMacroBlockType type) { attr_ = type; }
...
private:
  ...
  DISALLOW_COPY_AND_ASSIGN(KwMacroBlockCommonHeader);
};
} // blocksstable
} // oceanbase
