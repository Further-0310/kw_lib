...
KwTableReadInfo::~KwTableReadInfo()
{
  reset();
}

void KwTableReadInfo::reset()
{
  if (nullptr != allocator_ && nullptr != index_read_info_) {
    index_read_info_->~KwTableReadInfo();
    ...
    const bool is_index_read_info)
{
  ...
  if (OB_UNLIKELY(is_valid())) {
    ...
    int16_t extra_rowkey_cnt = 2;//storage::ObMultiVersionRowkeyHelpper::get_extra_rowkey_col_cnt();
    ...
    int64_t trans_version_col_idx = schema_rowkey_cnt; //若不对可能是+7和+8
    // storage::ObMultiVersionRowkeyHelpper::get_trans_version_col_store_index(schema_rowkey_cnt, true);
    int64_t sql_sequence_col_idx = schema_rowkey_cnt + 1;
    // storage::ObMultiVersionRowkeyHelpper::get_sql_sequence_col_store_index(schema_rowkey_cnt, true);
...
int KwTableReadInfo::build_index_read_info(
    common::ObIAllocator &allocator,
    const int64_t schema_rowkey_cnt,
    const bool is_oracle_mode,
    const common::ObIArray<ObColDesc> &cols_desc)
{
  int ret = OB_SUCCESS;
  ObSEArray<ObColDesc, 16> idx_col_descs;
  ...
  //   STORAGE_LOG(WARN, "Failed to add extra rowkey cols", K(ret));
  } else { // storage::ObMultiVersionRowkeyHelpper::add_extra_rowkey_cols(idx_col_descs)
    ObColDesc desc;
    for (int64_t i = 0; OB_SUCC(ret) && i < 2; ++i) {
      ...
      if (OB_FAIL(idx_col_descs.push_back(desc))) {
  ...
  if(OB_FAIL(ret)){
    ...
    STORAGE_LOG(WARN, "Failed to append index col desc", K(ret), K(idx_col_desc));
      ...
      = static_cast<KwTableReadInfo *>(allocator.alloc(sizeof(KwTableReadInfo))))) {
    ret = OB_ALLOCATE_MEMORY_FAILED;
    STORAGE_LOG(WARN, "Failed to alloc memory for index read info", K(ret), K(sizeof(KwTableReadInfo)));
  } else if (OB_ISNULL(index_read_info_ = new (index_read_info_) KwTableReadInfo())) {
...
int KwTableReadInfo::deserialize(
    common::ObIAllocator &allocator,
    const char *buf,
...
int64_t KwTableReadInfo::get_serialize_size() const
{
  int ret = OB_SUCCESS;
  int64_t len = 0;
...
int64_t KwTableReadInfo::to_string(char *buf, const int64_t buf_len) const
{
  int64_t pos = 0;
  J_OBJ_START();
