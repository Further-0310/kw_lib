...
KwSSTableMacroBlockHeader::KwSSTableMacroBlockHeader()
  : fixed_header_(),
...
void KwSSTableMacroBlockHeader::reset()
{
  fixed_header_.reset();
  column_types_ = nullptr;
  column_orders_ = nullptr;
...
int64_t KwSSTableMacroBlockHeader::to_string(char* buf, const int64_t buf_len) const
{
  int64_t pos = 0;
...
bool KwSSTableMacroBlockHeader::is_valid() const
{
...
KwSSTableMacroBlockHeader::FixedHeader::FixedHeader()
  : header_size_(0),
...
bool KwSSTableMacroBlockHeader::FixedHeader::is_valid() const
{
...
void KwSSTableMacroBlockHeader::FixedHeader::reset()
{
  header_size_ = 0;
  version_ = SSTABLE_MACRO_BLOCK_HEADER_VERSION_V1;
  magic_ = SSTABLE_MACRO_BLOCK_HEADER_MAGIC;
...
int KwSSTableMacroBlockHeader::serialize(char *buf, const int64_t buf_len, int64_t& pos) const
{
...
int KwSSTableMacroBlockHeader::deserialize(const char *buf, const int64_t data_len, int64_t& pos)
{
  int ret = 0;//OB_SUCCESS;
  if (OB_UNLIKELY(is_inited_)) {
    ret = -1;//OB_INIT_TWICE;
...
int64_t KwSSTableMacroBlockHeader::get_serialize_size() const
{
  return get_fixed_header_size() + get_variable_size_in_header(fixed_header_.column_count_);
}

int64_t KwSSTableMacroBlockHeader::get_fixed_header_size()
{
  return sizeof(FixedHeader);
}

int64_t KwSSTableMacroBlockHeader::get_variable_size_in_header(const int64_t column_cnt)
{
...
int KwSSTableMacroBlockHeader::init(
    ...
    int64_t *col_checksum)
{
  ...
  if (OB_UNLIKELY(is_inited_)) {
    ...
    fixed_header_.micro_block_data_offset_ = fixed_header_.header_size_
        + static_cast<int32_t>(KwMacroBlockCommonHeader::get_serialize_size());
