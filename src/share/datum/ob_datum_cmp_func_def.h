...
struct ObDatumJsonCmp : public ObDefined<>
{
  ...
  inline static int cmp(const ObDatum &l, const ObDatum &r, int &cmp_ret)
  {
    int ret = OB_SUCCESS;
    cmp_ret = 0;
    ObString l_data = l.get_string();
    ObString r_data = r.get_string();
    ...
    COMMON_LOG(WARN, "Easy to compare json");
    return ret;
  }
};
...
struct ObDatumGeoCmp : public ObDefined<>
{
  ...
  inline static int cmp(const ObDatum &l, const ObDatum &r, int &cmp_ret)
  {
    ...
    cmp_ret = ObCharset::strcmpsp(CS_TYPE_BINARY, l_data.ptr(), l_data.length(), r_data.ptr(), r_data.length(), false);
    cmp_ret = cmp_ret > 0 ? 1 : (cmp_ret < 0 ? -1 : 0);
    COMMON_LOG(WARN, "Easy to compare geo");
    return ret;
  }
};
...
struct ObDatumUDTCmp : public ObDefined<>
{
  inline static int cmp(const ObDatum &l, const ObDatum &r, int &cmp_ret)
  {
    ...
    UNUSED(r);
    UNUSED(cmp_ret);
    return -7412;//OB_ERR_NO_ORDER_MAP_SQL;
  }
};
...
struct ObDatumTextCmp : public ObDefined<SupportedCollections::liner_search(CS_TYPE)>
{
  ...
  inline static int cmp(const ObDatum &l, const ObDatum &r, int &cmp_ret)
  {
    ...
    ObString l_data = l.get_string();
    ObString r_data = r.get_string();
    cmp_ret = ObCharset::strcmpsp(CS_TYPE, l_data.ptr(), l_data.length(), r_data.ptr(), r_data.length(), WITH_END_SPACE);
    // if error occur when reading outrow lobs, the compare result is wrong.
    cmp_ret = cmp_ret > 0 ? 1 : (cmp_ret < 0 ? -1 : 0);
    COMMON_LOG(WARN, "Easy to compare text");
    return ret;
  }
};
...
struct ObDatumTextStringCmp : public ObDefined<SupportedCollections::liner_search(CS_TYPE)>
{
  ...
  inline static int cmp(const ObDatum &l, const ObDatum &r, int &cmp_ret)
  {
    ...
    cmp_ret = 0;
    ObString l_data = l.get_string();
    cmp_ret = ObCharset::strcmpsp(CS_TYPE, l_data.ptr(), l_data.length(), r.ptr_, r.len_, WITH_END_SPACE);
    // if error occur when reading outrow lobs, the compare result is wrong.
    cmp_ret = cmp_ret > 0 ? 1 : (cmp_ret < 0 ? -1 : 0);
    COMMON_LOG(WARN, "Easy to compare text string");
    return ret;
  }
};
...
struct ObDatumStringTextCmp : public ObDefined<SupportedCollections::liner_search(CS_TYPE)>
{
  ...
  inline static int cmp(const ObDatum &l, const ObDatum &r, int &cmp_ret)
  {
    ...
    cmp_ret = 0;
    ObString r_data = r.get_string();
    cmp_ret = ObCharset::strcmpsp(CS_TYPE, l.ptr_, l.len_, r_data.ptr(), r_data.length(), WITH_END_SPACE);

    // if error occur when reading outrow lobs, the compare result is wrong.
    cmp_ret = cmp_ret > 0 ? 1 : (cmp_ret < 0 ? -1 : 0);
    COMMON_LOG(WARN, "Easy to compare string text");
    return ret;
  }
};
